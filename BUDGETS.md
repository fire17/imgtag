...
> 🐧 marks the rows whose threshold is target-specific; 🖥 marks the handful that are
> dev/UI-only (B4, B14). `PROXY` prefixes any threshold derived from the M3 Max.
...
|---|---|---|---|---|---|
| B1 🐧 | "blazing fast … processing and indexing" | sustained e2e index throughput (files→searchable), CPU-only, **POLITE mode** (headline). *Sustained* = wall clock from process start to the manifest commit that makes the LAST image searchable, ÷ N; first 5% discarded as warm-up; shard flush, manifest commit and thumbnails all inside the timed region | PROXY: CORPUS-A ≥150 img/s · CORPUS-B ≥60 img/s · stretch (FULL, labeled) ≥180 = beat rclip's measured CoreML rate on pure CPU · ⌂-ub ≥10 img/s · **PRIMARY (🐧): NOT MEASURED** — locked only by a run on the real server; planning figure ≥10 img/s (photofield-ai did 20 on a 2014 6-core at full speed; we run half-cores at nice 10) · **HEAD-TO-HEAD GATE: red if slower than rclip on same corpus+machine+run** | `uv run imgtag bench index --corpus A,B --headtohead rclip` | provisional (proxy-only) |
| B2 | "time to process 10,000 images on cpu (tests scales to 100…)" | real 10,000-image wall time **+ projection fidelity** (what licenses the n=100 CI shortcut) | CORPUS-B t_10k ≤ 10000/B1_floor by construction (≤3min at ≥60 img/s) · CORPUS-A t_10k ≤2min · **projection gate: \|t_10k projected from n=100 − t_10k actual\| / t_10k actual ≤0.15**. Full 10k run required once per lock and once per darwin round; n=100 is CI-only · **🐧 target: NOT MEASURED — planning figure ≤25min POLITE (locks only on the real server)** | `uv run imgtag bench index --n 100` (CI) · `--n 10000` (lock/darwin) | provisional |
| B3 | "especially during inference/search … instantly" | search e2e warm-daemon latency over ≥200 DISTINCT never-before-seen queries (cache never pre-warmed with the test set), CORPUS-C | p50 ≤50ms · p95 ≤120ms · cache-HIT latency reported separately, never as the budget · daemon asserted resident (cold path is B13) · scan array asserted f32 · 100k synthetic-scale scan p95 ≤15ms (graceful-degradation claim, §1.1 measured 7.4ms) · **🐧 target: p50 ≤80ms · p95 ≤200ms — NOT MEASURED until the real server** | `uv run imgtag bench search --queries 200 --no-cache-prewarm` | provisional |
...
| B13 | "very very lightweight poc app" | server cold start → first searchable response (incl. model load, no daemon) @10k, f32 open-cost included | 🖥 dev ≤2s **warm page cache** · 🐧 target ≤5s **cold disk** (page cache purged — `sudo purge`/`vmtouch -e`, documented in the bench) and ≤2s warm · both printed; **the cold number is the one quoted in any public claim** · cold start is rare by design (long-lived daemon, ADR-13) · ORT-optimized graph cached (`optimized_model_filepath`) so session creation is not re-paid | `uv run imgtag bench coldstart` | provisional |
...
> B21 restart survival tightened.** Full adjudication: `.deify/reviews/ADJUDICATION.md`;
