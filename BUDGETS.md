...
| B25 | "even if we have 100 tracks, the times for indexing and inferencing should remain relatively the same" | track marginal cost + dedicated-FLOPs share + sidecar integrity | embedding-track marginal ≤0.5% each, ≤5% total @100 simulated · Σ dedicated-model FLOPs ≤30% of encoder · sidecar row-alignment 100% · tier derivation deterministic | `uv run imgtag bench tracks` | provisional |
