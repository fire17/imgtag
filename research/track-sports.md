...
2. **Contact-sports recall is weak** (§8) — strengthen `martial arts` prompts and re-measure
   ...
   2026-07-22: track-violence is building dataset **`vslices`** (target 1856 — real
   ring/dojo/mat boxing/martial-arts/wrestling/fencing + team-contact & confusables,
   pecore-s16-384-fp32). ⚠️ PARTIAL (~984/1856) — its index was terminated mid-build; it
   completes after the quiet-window ALL-CLEAR, then track-violence pings when WHOLE. DO NOT
   score it until that ping (a partial slice gives wrong per-class rates). STAGED then: run
   `SportsHead.score` over `store.open_snapshot("vslices")` for REAL per-slice match rates
   (replaces the weak-keyword 0.102/0.146). If a targeted prompt round lifts boxing recall,
   ping track-violence FIRST (freeze contract) to upgrade their §7 to "measured".
